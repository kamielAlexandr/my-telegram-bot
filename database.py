...
# --- КОНСТАНТЫ ИГРЫ (БАЛАНС) ---
RACES = {
    "human": {
        "name": "👨 Человек",
        "desc": "Сбалансированные бойцы, способные адаптироваться к любой ситуации.",
        "strength": 8, "agility": 8, "intelligence": 8, "vitality": 8,
        "health_mult": 10, "mana_mult": 5,
        "ability": "Адаптивность (Бонус к защите)"
    },
    "elf": {
        "name": "🧝 Эльф",
        "desc": "Древний народ, владеющий луком и магией леса.",
        "strength": 5, "agility": 12, "intelligence": 10, "vitality": 5,
        "health_mult": 8, "mana_mult": 8,
        "ability": "Меткий глаз (Высокий шанс крита)"
    },
    "dwarf": {
        "name": "⚒️ Дварф",
        "desc": "Жители подземелий, крепкие как скала.",
        "strength": 10, "agility": 4, "intelligence": 6, "vitality": 12,
        "health_mult": 12, "mana_mult": 3,
        "ability": "Каменная кожа (Высокое HP)"
    },
    "orc": {
        "name": "👹 Орк",
        "desc": "Яростные воины, полагающиеся на грубую силу.",
        "strength": 13, "agility": 6, "intelligence": 3, "vitality": 10,
        "health_mult": 11, "mana_mult": 2,
        "ability": "Ярость (Бонус к урону)"
    }
}
...
def init_db():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            """)
            conn.commit()
            print("✅ База данных инициализирована (Атмосфера восстановлена)")
    finally:
        conn.close()

def apply_regeneration(character):
    """Регенерация: 5% HP и MP в минуту пока герой отдыхает"""
    ...
    # Если полное ХП/МП, просто сдвигаем таймер
...
def add_rewards(user_id, exp, gold):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT level, experience, stat_points FROM player_characters WHERE user_id=%s", (user_id,))
            data = cursor.fetchone()
            if not data: return
            
            cur_lvl, cur_exp, cur_pts = data
            new_exp = cur_exp + exp
            new_lvl = cur_lvl
            
            # Сложная кривая опыта: Уровень * 150 (накопительная)
            while True:
                needed = (new_lvl * (new_lvl + 1) * 150) // 2
                if new_exp >= needed:
                    new_lvl += 1
                    cur_pts += 3 # 3 очка за уровень
                else:
                    break
            ...
            if new_lvl > cur_lvl:
                update_sql += ", level=%s, stat_points=%s, health=max_health, mana=max_mana"
                params.extend([new_lvl, cur_pts])
...
# Магазин и Инвентарь (упрощенная реализация для экономии места, но функциональная)
